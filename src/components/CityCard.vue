  ...
  <div class="flex py-6 px-3 bg-weather-secondary rounded-md shadow-md cursor-pointer">
    ...
    <div class="flex flex-col gap-2">
      <p class="text-3xl self-end">
        {{ Math.round(celsiusFunction(city.weather.main.temp)) }}&deg;
      </p>
      <div class="flex gap-2">
        <span class="text-xs">
          Max :
          {{ Math.round(celsiusFunction(city.weather.main.temp_max)) }}&deg;
        </span>
        <span class="text-xs">
          Min :
          {{ Math.round(celsiusFunction(city.weather.main.temp_min)) }}&deg;
        </span>
      </div>
    </div>
  </div>
